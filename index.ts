...
const createPipes = y =>
  (random =>
    Array.from(Array(gameSize).keys())
      .map(e => gamePipe(e, y))
      .filter(e => e.x < random || e.x > random + 1))(
    Math.floor(Math.random() * Math.floor(gameSize))
  );

const gamePipes$ = interval(200).pipe(
  scan < any,
  any >
    (acc =>
...
const fall = xPos => (xPos < gameSize - 1 ? (xPos += 1) : xPos); // : gameSize - 1
const bird$ = merge(interval(500), fromEvent(document, 'keydown')).pipe(
  scan < any, 
  any >
    ((xPos, curr) => (curr.key == 'ArrowUp' ? fly(xPos) : curr.key == 'ArrowDown' ? fall(xPos) : xPos), gameSize - 5) 
);
